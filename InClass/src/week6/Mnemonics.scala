...
object mnemonics extends App {
  ...
  val in = Source.fromFile("/Users/manasapte/words.txt")
  ...
  /** invert mnem map to give a map from chars to nums */
  val charCode: Map[Char, Char] = for( (num, chars) <- mnem; chr <- chars) yield chr -> num
  ...
  println(translate("7225247386"))                         //> res0: Set[String] = Set(sack air fun, pack ah re to, pack bird to, Scala ir
